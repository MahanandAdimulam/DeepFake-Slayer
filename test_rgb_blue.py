...
class FacialForgeryDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if len(image_array.shape) == 3:  # Check if the image has color channels (RGB)
            image_array[:, :, 2] = np.clip(image_array[:, :, 2] * self.num, 0, 255)
...
MODEL_DIR = '/shared/rc/defake/Deepfake-Slayer/models_binary/test_rgb_blue/'
...
RESDIR = '/shared/rc/defake/Deepfake-Slayer/models_binary/test_rgb_blue/xcp_reg/'#/content/models/xcp_tmp/results_0.mat /content/models/test/xcp_reg
...
def main():
  ...
  for num in rgb_variations:
    model_dir = f'/shared/rc/defake/Deepfake-Slayer/models_binary/test_rgb_blue_{num}/xcp_reg/'
    if not os.path.exists(model_dir):
      os.makedirs(model_dir)
  ...
  #   output = '/content/gdrive/MyDrive/shared/rc/defake/WildDeepfakes/output/test'
    ...
    if not os.path.exists(output):
          os.makedirs(output)
    file = open(f'{output}/test_rgb_blue_{num}.txt','w')
    ...
    print(f'Testing complete for rgb blue {num}')
    file.close()
