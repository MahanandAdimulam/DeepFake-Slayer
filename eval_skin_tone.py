...
def eval():
  ...
  for skin_tone in skin_category:
    ...
    file.write('Found {0} total images with scores.\n'.format(filtered_data['lab'].shape[0]))
    file.write('  {0} results are real images.\n'.format((filtered_data['lab'] == 0).sum()))
    file.write('  {0} results are fake images.\n'.format((filtered_data['lab'] == 1).sum()))

    # Compute the performance numbers
    if filtered_data['mask'].shape[0] !=0 and filtered_data['msk'].shape[0] !=0 and filtered_data['lab'].shape[0] !=0 and filtered_data['pred'].shape[0] !=0:
      PRED_ACC = (filtered_data['lab'] == filtered_data['pred']).astype(np.float32).mean()
