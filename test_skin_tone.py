...
class FacialForgeryDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        image, mask, skin = self.mask_list[idx]
        label = 0
        ...
        return {'image': image, 'label': label, 'msk': mask, 'skin': torch.tensor(skin)}
...
class Model:
  ...
  def load(self, epoch, model_dir):
    ...
    if os.path.exists(filename):
      state = torch.load(filename)#, map_location="cpu")#When we do not use GPU
...
# MODEL_DIR = '/content/models/test/'
# MODEL_DIR = '/shared/rc/defake/Deepfake-Slayer/models/test/'
MODEL_DIR = '/shared/rc/defake/Deepfake-Slayer/models_binary/test_Skin/'
BACKBONE = 'xcp'
MAPTYPE = 'reg'#'tmp'
BATCH_SIZE = 200
MAX_EPOCHS = 100
...
# MODEL.load(18,'/shared/rc/defake/Deepfake-Slayer/models/FFDmodel/')
...
# RESDIR = '/content/models/test/xcp_reg/'#/content/models/xcp_tmp/results_0.mat /content/models/test/xcp_reg
RESDIR = '/shared/rc/defake/Deepfake-Slayer/models_binary/test_Skin/xcp_reg'#'/shared/rc/defake/Deepfake-Slayer/models/test/xcp_reg'#/content/models/xcp_tmp/results_0.mat /content/models/test/xcp_reg
...
def load_skin_colors(csv_file):
    skin_colors = {}
    with open(csv_file, 'r') as file:
        reader = csv.DictReader(file)
        for row in reader:
            idx = int(row['Image_Index'])  # Assuming 'idx' is the column name in your CSV
            skin_color = int(row['Skin_Color_Encoded'])  # Assuming 'skin_color' is the column name
            skin_colors[idx] = skin_color
    return skin_colors

# Update the test list to include skin color
def update_test_list(test_list, skin_colors):
    updated_list = []
    for idx, (image_path, mask_path) in enumerate(test_list):
        skin_color = skin_colors.get(idx, -1)  # Default to None if not found
        updated_list.append((image_path, mask_path, skin_color))
    return updated_list
...
def main():
  ...
  print(f"Test list: {test_list[:5]}")
  skin_colors = load_skin_colors(csv_file)
  test_list_updated = update_test_list(test_list, skin_colors)
  print(f"Test list Updated: {test_list_updated[:5]}")

  test_dataset = FacialForgeryDataset(test_list_updated, transform, transform_mask)
  batch_size = 16
  shuffle = True
  ...
  if not os.path.exists(output):
        os.makedirs(output)
  file = open(f'{output}/test_with_skin.txt','w')
